...
def get_comment(fields, index1, index2):
    fs = ",".join(fields)             
    sql = "SELECT %s FROM c_lcs_game_comment_summary where cast(game_id as signed)>=%s order by cast(game_id as signed) asc limit %s" % (fs, index1, index2)
...
def last_comment(fields, index1, index2):
    fs = ",".join(fields)             
    sql = "select * from (SELECT %s FROM c_lcs_game_comment_summary where cast(game_id as signed)<=%s order by cast(game_id as signed) desc limit %s) a  order by cast(a.game_id as signed) asc" % (fs, index1, index2)
    conn = pymysql.connect(host='127.0.0.1',port=3306,user='root',password='eXYhzAWjyvy8grwM',db='game_process',charset='utf8mb4')
    # 创建mysql 游标
    cursor = conn.cursor()
    cursor.execute(sql)
